...
if (toggleSwitch && switchLabel) {
  ...
  toggleSwitch.addEventListener("change", () => {
    ...
    if (isChecked) {
      ...
      hideMenu();
      // Petit délai pour éviter le jump si la souris est encore dessus
      setTimeout(() => {
    ...
    function isMenuAlwaysVisible() {
      return localStorage.getItem("menuAlwaysVisible") === "true";
    }
  
    function areMouseEventsEnabled() {
      return localStorage.getItem("menuMouseEventsEnabled") !== "false";
    }
  
    console.log("Menu always visible:", isMenuAlwaysVisible());
    console.log("Mouse events enabled:", areMouseEventsEnabled());
  
    function showMenu() {
      items.forEach(el => el.style.transform = "translateX(0px)");
      if (menuTag) menuTag.style.transform = "translate(-150px)";
      menu.style.opacity = "1";
    }
  
    function hideMenu() {
      items.forEach(el => el.style.transform = "translateX(-150px)");
      if (menuTag) menuTag.style.transform = "translate(0px)";
    ...
    if (closeMenu) {
      closeMenu.addEventListener("click", () => {
        console.log("Fermeture manuelle du menu");
        hideMenu();
        localStorage.setItem("menuAlwaysVisible", "false");
        localStorage.setItem("menuMouseEventsEnabled", "true");
  
        setTimeout(() => {
          menu.addEventListener("mouseover", showMenu);
          menu.addEventListener("mouseout", hideMenu);
          console.log("Mouse events réactivés après délai");
        }, 1000);
      });
    }
  }
      ...
      const html = `
        <div class="card-post" data-id="${id}" id="card">
          <div class="card-title">
            ...
            <h1 class="fullname">Paul Destaing</h1>
          </div>
    ...
    <div class="response-posted" data-id="${id}">
      <div class="response-card">
        <div class="response-title">
          <div class="profil">P</div>
          <h1 class="fullname-response">Paul Destaing</h1>
        </div>
